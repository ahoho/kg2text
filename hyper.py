...
def create_git_log(path=None, include_untracked=False):
    ...
    If it's dirty, stash the changes and record the stash
    """
    repo = Repo(path=path, search_parent_directories=False)
    if repo.is_dirty() or (include_untracked and repo.untracked_files):
        raise ValueError(f"git repo at {repo.working_dir} is dirty. Please commit.")
    
    commit = repo.head.commit # head
    log = (
        f"On branch {repo.active_branch}\n"
...
def hyper(
    ...
    for c in configs:
        ...
        save_text(git_log, Path(output_dir, "git.log"))
