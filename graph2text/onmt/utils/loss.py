...
def kd_loss(log_prob, logit_values, logit_indices, temperature, mask):
    """
    Loss and temperature scaling
    ...
    # TODO: Decide whether to experiment with this
    T = temperature
    probs = F.softmax(logit_values/T, dim=-1)
    loss = -(log_prob.gather(dim=-1, index=logit_indices) * probs)[mask].sum()
    return loss


class KDLossCompute(LossComputeBase):
    """
    ...
    def __call__(self, outputs, logit_values, logit_indices, target, normalization,
                 temperature, alpha):
        ...
        if alpha > 0:
            logit_values = self._bottle(logit_values)
            logit_indices = self._bottle(logit_indices)

            loss_kd = kd_loss(log_prob, logit_values, logit_indices,
                              temperature, non_pad)
            # TODO: scale loss_kd by temp?
